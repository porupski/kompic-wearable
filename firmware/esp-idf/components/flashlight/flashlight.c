...
esp_err_t flashlight_init(void)
{
    ...
    // Force GPIO41 to a defined LOW state before LEDC takes over: internal
    // pull-DOWN on, pull-up OFF. LEDC drives the pin high/low actively, so
    // the pulldown only matters in the small windows before init runs and
    // during deep sleep (where the LED must stay dark).
    gpio_pullup_dis(FLASHLIGHT_GPIO);
    gpio_pulldown_en(FLASHLIGHT_GPIO);
