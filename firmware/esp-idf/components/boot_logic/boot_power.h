 ...
 *   - GPIO0  = DRV_EN: haptic enable strap. Driven HIGH after boot
 *              (DRV2605 EN pin is active-HIGH; LOW = shutdown).
 *   - No GPIO power latch. The BQ25619's BATFET is the only off path.
 *
 * MUST be the first thing called in app_main() (drives GPIO0 HIGH before
 * any sensor / display init runs so the DRV2605 is out of shutdown when
 * drv2605_init() writes its config registers).
 */
