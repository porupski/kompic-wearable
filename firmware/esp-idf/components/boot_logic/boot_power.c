 ...
 *   - GPIO0 driven HIGH so DRV2605 is enabled (EN pin is active-HIGH).
 *
...
void boot_power_init(void)
{
    ...
    // DRV2605 EN pin is active-HIGH. Previous version drove LOW, which put
    // the chip in shutdown -- auto-cal timed out, no vibration. Arduino
    // sketch (7_demo_field_capture) writes HIGH here; matched.
    gpio_set_level(GPIO_DRV_EN, 1);
    ESP_LOGI(TAG, "DRV_EN driven HIGH (GPIO%d)", GPIO_DRV_EN);
