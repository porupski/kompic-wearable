...
esp_err_t bq25619_init(i2c_port_t i2c_num)
{
    ...
    //                    may have left this set; clear so charging can resume
    //                    even if the operator plugged USB back without a
    //                    full VBUS-removal cycle.
    const uint8_t IINDPM_TS_IGNORE  = (1 << 6);
    ...
    //    after VBUS presence unless the host kicks the watchdog periodically.
    uint8_t timer = 0;
    if (bq25619_read_reg(i2c_num, BQ25619_REG_TIMER, &timer) == ESP_OK) {
        bq25619_write_reg(i2c_num, BQ25619_REG_TIMER, timer & (uint8_t)~TIMER_WD_MASK);
    }

    // 3. Clear BATFET_DIS if a prior session's ship-mode left it latched.
    //    Ship mode sets BATFET_DIS + BATFET_RST_WVBUS; the latter auto-
    //    re-enables BATFET on VBUS removal, but if the operator plugs USB
    //    without a full unplug cycle, the flag stays set and charging is
    //    blocked. Explicit clear at every boot side-steps this.
    uint8_t misc = 0;
    if (bq25619_read_reg(i2c_num, BQ25619_REG_MISC, &misc) == ESP_OK) {
        if (misc & MISC_BATFET_DIS) {
            bq25619_write_reg(i2c_num, BQ25619_REG_MISC, misc & (uint8_t)~MISC_BATFET_DIS);
            ESP_LOGW(TAG, "cleared latched BATFET_DIS (previous ship-mode -- charging restored)");
        }
    }
