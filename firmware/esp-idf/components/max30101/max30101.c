...
// Wrist-worn PPG optimisation (see Stage_6_Build_Report.md § 7.4):
// green-primary, higher current, Red/IR off. Wrist perfusion index is
// ~50x lower than fingertip; we need more return light AND we drive the
// beat detector from GREEN because green (~525 nm) has the strongest AC
// modulation through skin (oxyhemoglobin absorption peak). Red/IR only
// come on for SpO2 -- until then, off saves LED current and eliminates
// cross-channel optical leakage into the green PD path.
#define HR_LED_RED_CURRENT          0x00
#define HR_LED_IR_CURRENT           0x00
#define HR_LED_GREEN_CURRENT        0x7F   // ~30 mA -- 4x the old 0x1F/~7 mA
                                         ...
                                         uint8_t green_pa)
{
    ...
    if (ret != ESP_OK) return ret;
    // FIFO_CONFIG: SMP_AVE=8 (hardware 8x boxcar), FIFO_ROLLOVER_EN=1.
    // Boxcar smoothes LED shot noise / mains-hum aliasing without
    // slowing the FIFO too much. 100 Hz SR / 8 avg = 12.5 Hz effective
    // FIFO update -- fast enough to catch the ~100 ms cardiac upstroke.
    ret = write_reg(port, MAX30101_REG_FIFO_CONFIG,
                    (uint8_t)(0x50u | (MAX30101_SMP_AVE_8 << 5)));
    if (ret != ESP_OK) return ret;
    // SPO2_CFG: ADC full-scale 16384 nA (was 4096 nA -- more headroom
    // against LED->PD crosstalk on the higher green current), SR = 100 Hz,
    // PW = 411 us (max integration per sample).
    uint8_t spo2_cfg = (uint8_t)((MAX30101_ADC_16384 << 5) |
                                  (MAX30101_SR_100    << 2) |
                                   MAX30101_PW_411);
    ret = write_reg(port, MAX30101_REG_SPO2_CONFIG, spo2_cfg);
    if (ret != ESP_OK) return ret;
...
void task_hr_fn(void *arg)
{
    ...
    while (1) {
        ...
        if (en && !sensor_awake) {
            ...
            if (xSemaphoreTake(g_i2c_mutex, pdMS_TO_TICKS(200)) == pdTRUE) {
                // Wrist-worn PPG: green-primary, Red/IR off. See § 7.4 of
                // Stage_6_Build_Report.md for the rationale.
                max30101_setup_multi_led_mode(MAX30101_I2C_PORT,
                                              HR_LED_RED_CURRENT,     // 0x00
                                              HR_LED_IR_CURRENT,      // 0x00
                                              HR_LED_GREEN_CURRENT);  // 0x7F
                xSemaphoreGive(g_i2c_mutex);
            }
        ...
        if (xSemaphoreTake(g_i2c_mutex, pdMS_TO_TICKS(50)) == pdTRUE) {
            ...
            for (uint8_t i = 0; i < avail; i++) {
                ...
                if (max30101_read_fifo(MAX30101_I2C_PORT, &s, true) == ESP_OK && s.valid) {
                    uint32_t now_ms = (uint32_t)(esp_timer_get_time() / 1000ULL);
                    // Beat detector on GREEN channel (was IR). Green has the
                    // strongest AC modulation through skin -- what commercial
                    // watches use for HR. See § 7.4 of Stage_6_Build_Report.md.
                    bool b = max30101_check_for_beat(&detector, s.green, now_ms);
                    beat   = beat || b;
                    got_s  = true;
                    samples_since_wake++;
                    (void)HR_IR_LOG_SAMPLES;   // hook for re-enabling raw-channel log if needed
                }
            }
            xSemaphoreGive(g_i2c_mutex);
        }
        ...
        if (avail == 0) {
            ...
            if (empty_fifo_ticks >= HR_EMPTY_FIFO_REINIT_TICKS) {
                ...
                vTaskDelay(pdMS_TO_TICKS(10));
                if (xSemaphoreTake(g_i2c_mutex, pdMS_TO_TICKS(200)) == pdTRUE) {
                    // Recovery re-init: same green-primary multi-LED as wake.
                    max30101_setup_multi_led_mode(MAX30101_I2C_PORT,
                                                  HR_LED_RED_CURRENT,
                                                  HR_LED_IR_CURRENT,
                                                  HR_LED_GREEN_CURRENT);
                    xSemaphoreGive(g_i2c_mutex);
                }
                vTaskDelay(pdMS_TO_TICKS(HR_FIFO_SETTLE_MS));
                max30101_beat_detector_init(&detector);
