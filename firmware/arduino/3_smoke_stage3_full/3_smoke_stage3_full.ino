/**
 * 3_smoke_stage3_full.ino — Kompic Mk I, full-board smoke test
 *
 * Refactored 2026-07-05 as part of the Stage 5 close-out. Renamed from
 * 3_smoke_test_mk1_hotair in the pre-refactor legacy set. Additions since
 * the original 2026-06-20 write-up:
 *   - Ship-mode double-click handler (2026-07-05 port from 4_demo_mk1).
 *   - Summary print reordered to run AFTER SD init (Stage 3 § 4.6 mitigation
 *     no longer needed now that SD is reliable per Stage 4).
 * See hardware/Reflow_info/Stage_5_Build_Report.md for context.
 *
 ...
 *   - Auto-cal at boot with the ELV1411A profile (RATED=0x49, OD_CLAMP=0x60).
 *     Post-Stage-5 chip swap: cal PASSes in ~1.1 s (STATUS=0xE0,
 *     COMP=0x07, BEMF=0x8F, no OC_DETECT). Effects play with a firm
 *     tactile click at each encoder detent.
 *
...
#define WS_BLIP_LEVEL        26   // ship-mode 2 s red countdown peak

// ── Button timing (ship-mode capable) ────────────────────────────────────────
...
void setup() {
    ...
    // ── SD card (1-bit SDMMC) ────────────────────────────────────────────────
    // Historical note: Stage 3 § 4.6 documented an ipc1 stack-canary panic when
    // SD_MMC.begin() failed on a bad card -- the async cleanup would blow the
    // canary mid-summary-print. The sketch worked around it by deferring SD to
    // the very last statement of setup(). Post-Stage-4 (SD verified 65/65 at
    // 20 MHz), that panic is no longer observed in normal operation, so SD
    // init now runs BEFORE the summary, giving the summary an accurate sd_ok
    // reading. The synchronous SD_MMC.end() on failure is kept as belt-and-
    // braces for the bad-card case.
    ...
    Serial.println("\n[BTN ] Single-click GPIO16 = toggle flashlight; double-click = ship mode");
...
// ── Button state machine (ship-mode capable) ─────────────────────────────────
// Ship-mode double-click path is required on every sketch flashed to iv7.1:
// the battery is permanently attached and this is the only firmware path
// that powers the board off. See Stage 3 § 8.1 for the R12 zombie-wake
// discovery that made this pattern mandatory. Single click keeps this
// sketch's original behaviour (toggle the flashlight sine breathe).
typedef enum {
    BTN_IDLE,
    BTN_PRESSED,
    BTN_WAIT_DBL,
...
static void on_single_click(void) {
    ...
    Serial.printf("[BTN ] single -> LED %s\n", led_on ? "ON" : "OFF");
}
...
static void enter_ship_mode(void) {
    ...
    ship_mode_countdown();
    // BQ25619 sees /QON = button. If it's still LOW here, the ship-mode
    // write would race t_SHIPMODE and trigger a zombie wake. Bail cleanly.
    ...
    r07_new |=  BQ_BATFET_RST_WVBUS;   // required when USB is plugged in
    r07_new &= ~BQ_BATFET_DLY;
    r07_new &= ~BQ_BATFET_RST_EN;      // block the 8-12 s QON-hold reset path
    ...
    if (low != prev_low && (now - last_change) >= BTN_DEBOUNCE_MS) {
        ...
        if (low) {
            ...
            if (state == BTN_PRESSED) {
                ...
                // Fire ship-mode on RELEASE of the second press so /QON is
                // HIGH when we write BATFET_DIS.
                state = BTN_IDLE;
                on_double_click();
            }
        }
