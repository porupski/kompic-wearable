/**
 * 4_demo_flashlight.ino -- Kompic Mk I demo #1: wearable flashlight
 *
 * Refactored 2026-07-05 as part of the Stage 5 close-out. Renamed from
 * 4_demo_mk1 in the pre-refactor legacy set. See
 * hardware/Reflow_info/Stage_5_Build_Report.md for context.
 *
 * Built on the stage-3 hot-air smoke test. Strips MAX30101, TMP117 and the
 * SD card; keeps everything else that was confirmed working in stage 3,
 * drops it into the lowest power state we can, and exposes a small
 * interactive UI suitable for taking the board out of the lab on a
 * 400 mAh cell.
 *
 * UI
 ...
 *   4. DRV2605L auto-cal runs every boot with the ELV1411A profile
 *      (RATED_VOLTAGE=0x49, OD_CLAMP=0x60). Cal completes in ~1.1 s on the
 *      fresh chip fitted in Stage 5 -- STATUS=0xE0, COMP=0x07, BEMF=0x8F,
 *      no OC_DETECT. Fallback path stays as internal-trigger + LRA library
 *      at 150 Hz open-loop in the event of DIAG=1 or timeout.
 *   5. Flashlight single strobe up to 50 % then off; RGB lights up
 *      half-power and starts roaming hue.
 *
 * Pins / addresses match stage-3; see top of 3_smoke_stage3_full.ino
