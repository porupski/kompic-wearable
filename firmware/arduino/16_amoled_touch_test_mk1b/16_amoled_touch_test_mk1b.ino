/*
 * 16_amoled_touch_test_mk1b — CO5300 2.06" AMOLED + CST9217 touch bring-up
 * Target: Kompic Mk1b iv8.0 (ESP32-S3-WROOM-1U-N16R8)
 *
 * Purpose: fall-back sketch when the ESP-IDF main firmware boots but the panel
 * stays dark on Mk1b. Isolates the display + touch drivers from LVGL, the
 * boot orchestration, and the ESP-IDF I2C stack. If this sketch shows the
 * color-test pattern and stamps on touch, the panel wiring is fine and the
 * bug is in the ESP-IDF integration (co5300 init sequence, QSPI clocks,
 * LVGL flush cb, or CST9217 probe timing). If this sketch is ALSO dark, the
 * fault is in the FPC / receptacle / panel itself and Ivan has a bench debug
 * job before any firmware change matters.
 *
 * ── Diff vs 15_amoled_touch_test (iv7.1) ────────────────────────────
 * ALL display + touch pins are unchanged (master pinout v20 rules on both
 * boards). This sketch is a header-banner-only fork; the driver code is
 * identical to sketch 15.
 *
 * Notable Mk1b context that is NOT reflected in pin code:
 *   - GPIO18 was the iv7.1 bench prototype's Vbat divider (ADC2_CH7 override).
 *     Mk1b routes GPIO18 to MAX-M10S GPS RX per master pinout. This sketch
 *     does not touch GPIO18 either way.
 *   - Mk1b currently ships to Ivan's bench WITHOUT a battery. USB is the only
 *     power source. The double-click → BQ ship-mode call below still runs
 *     but with no battery attached the BATFET disable path has no visible
 *     effect (USB stays up regardless). Left in for parity with sketch 15
 *     and so this same sketch works after Ivan attaches a cell.
 *
 ...
 *   - GPIO3 (DISP_RST) is a JTAG-source strap; safe as an output.
 *   - TE (GPIO45) not read here; the ESP-IDF path uses it for tearing sync.
 */
...
// ════════════════════════════════════════════════════════════════════
//  PINS — Kompic_Pinout_MASTER_v20 (identical on iv7.1 and Mk1b)
// ════════════════════════════════════════════════════════════════════
...
#define DISP_RST     3
// #define QSPI_TE   45   // TE routed but not used in this sketch

#define TOUCH_SDA    1   // I2C bus 1 (shared with LSM/TMP117/MAX30101/RTC)
#define TOUCH_SCL    2
#define TOUCH_INT    6
#define TOUCH_RST   44
...
#define SDA_BUS2     4   // BQ25619 (and MAX17048 on Mk1b) live here
#define SCL_BUS2     5
#define BQ_ADDR     0x6A
...
static void touch_init() {
  ...
  // Wire.begin() runs in setup() before this. Touch shares bus 1 with the
  // main sensor set on both iv7.1 and Mk1b.
}

// ════════════════════════════════════════════════════════════════════
//  BQ25619 SHIP MODE
//  On Mk1b (no battery yet) this is a no-op — USB stays up regardless.
//  Left in so the same sketch works once Ivan attaches a cell.
// ════════════════════════════════════════════════════════════════════
...
void setup() {
  Serial.begin(115200);
  delay(400);
  Serial.println("\n=== 16_amoled_touch_test_mk1b (Mk1b iv8.0) ===");

  pinMode(BUTTON_PIN, INPUT_PULLUP);

  // Bus 1: touch + sensor bus
  Wire.begin (TOUCH_SDA, TOUCH_SCL, 400000);
  // Bus 2: BQ25619 (and, on Mk1b, MAX17048 fuel gauge -- pinged for record)
  ...
  if (i2c_ping(Wire1, 0x36)) {
    Serial.println("  MAX17048  0x36 : ACK  (fuel gauge present; no cell attached)");
  } else {
