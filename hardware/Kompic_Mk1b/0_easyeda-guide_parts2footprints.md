...
mamba activate pcb_dev
cd ~/path/to/kompic-wearable/hardware/Kompic_Mk1b

# Import all parts
grep -v '^\s*#' parts.txt | awk '{print $1}' | while read id; do
  echo "=== Fetching $id ==="
  easyeda2kicad --full --lcsc_id "$id" --output ./Kompic_Mk1b_library/Kompic_Mk1b
done

# Fix 3D model paths
sed -i 's|"/Kompic_Mk1b_library/|"${KOMPIC_MK1B_LIB}/|g' ./Kompic_Mk1b_library/Kompic_Mk1b.pretty/*.kicad_mod

# KiCad Configure Paths:
#   KOMPIC_MK1B_LIB → ~/path/to/kompic-wearable/hardware/Kompic_Mk1b/Kompic_Mk1b_library
```
